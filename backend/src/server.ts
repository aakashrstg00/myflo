...
app.get('/api/predictions', (req, res) => {
  ...
  let averagePeriodLength = Math.round(db.data.user.history.slice(0, -1).reduce((sum: number, c: any) => sum + (c.length_days || 0), 0) / cycleCount);
...
app.post('/api/check-in', async (req, res) => {
  const completedCycles = db.data.user.history.filter((c: any) => c.end_date !== null && c.length_days !== null);
  ...
  const averagePeriodLength = Math.round(completedCycles.reduce((sum: number, c: any) => sum + (c.length_days || 0), 0) / completedCycles.length);

  const incompleteCycle = db.data.user.history.find((c: any) => !c.end_date);
